...
!AZ TEST:
!write(*,*) "****** Scalar Cls. Calling cmbmain"
!AZ TEST.
        if (global_error_flag==0) call cmbmain
!AZ TEST:
!write(*,*) "****** Scalar Cls. cmbmain done"
!AZ TEST.
...
!AZ TEST.
    if (Params%WantCls .and. Params%WantTensors) then
        ...
        call CAMBParams_Set(P)
!AZ TEST:
!write(*,*) "****** Tensor Cls. Calling cmbmain"
!AZ TEST.
        if (global_error_flag==0) call cmbmain
        if (global_error_flag/=0) then
            if (present(error)) error =global_error_flag
...
!AZ TEST.
    if (Params%WantCls .and. Params%WantVectors) then
        ...
        call CAMBParams_Set(P)
!AZ TEST:
!write(*,*) "****** Vector Cls. Calling cmbmain"
!AZ TEST.
        if (global_error_flag==0) call cmbmain
        if (global_error_flag/=0) then
            if (present(error)) error =global_error_flag
...
!AZ TEST.
    ...
    call CAMBParams_Set(P)
!AZ TEST:
!write(*,*) "****** Transfer functions. Calling cmbmain"
!AZ TEST.
    if (global_error_flag==0) call cmbmain
    if (global_error_flag/=0) then
        if (present(error)) error =global_error_flag
