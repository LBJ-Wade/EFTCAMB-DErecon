...
!write(*,*) "next switch", next_switch, "EFTturnOnTime:", EV%EFTturnOnTime
...
!write(*,*) "turning EFT on"
    ...
    logical  :: IsNaN
    !> EFT sources: other quantities that are needed to compute the sources
    real(dl) :: EFTetakdot, EFTsigmadotdot, EFT_PsiDot, EFT_PhiDot, EFT_Phi, EFT_Psi
    ! EFTCAMB MOD END
    ...
    !write(*,*) "output: a, k, y = ", y(1), EV%k_buf, y
...
!> AZ MOD START
!if (k.ge. 1.108E-003 .and. k .le. 1.109E-003) then
!write(1,*) a, y
!end if
!> AZ MOD END
    ...
    if (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive) then
        ...
        !> EFTCAMB sources: computation of the quantites needed to construct the EFT sources
        !> First calculate etakdot
        EFTetakdot =  ( 0.5_dl*dgq/(1._dl+EV%eft_cache%EFTOmegaV) + k2/3._dl*EV%eft_cache%EFTeomF/CP%eft_par_cache%h0_Mpc &
            & + k2/3._dl*(EV%eft_cache%EFTeomU-EV%eft_cache%EFTeomX)*z )/EV%eft_cache%EFTeomX

        !> Then calculate sigmadotdot
        EFTsigmadotdot = 1._dl/EV%eft_cache%EFTeomX*(-2._dl*(1._dl+EV%eft_cache%EFTeomV)*(EV%eft_cache%Hdot*sigma+adotoa*EFTsigmadot)&
            & -2._dl*adotoa*sigma*EV%eft_cache%EFTeomVdot +EFTetakdot &
            & +a*adotoa*EV%eft_cache%EFTOmegaP/k*dgpi/(1._dl+EV%eft_cache%EFTOmegaV)**2&
            & -1._dl/(1._dl+EV%eft_cache%EFTOmegaV)*(2._dl*adotoa*dgpi/k+diff_rhopi/k)&
            & -EV%eft_cache%EFTeomXdot*EFTsigmadot+EV%eft_cache%EFTeomNdot/CP%eft_par_cache%h0_Mpc)

        !> Then compute the remaining quantities for EFTCAMB sources
        ...
        EFT_PhiDot = +EFTetakdot/k - adotoa*EFTsigmadot/k -EV%eft_cache%Hdot*sigma/k

        !> adding a few things for debug
...
!> AZ MOD START
!> output of the lensing potential and the ISW effect in fort.1 file
if ((k .ge. 0.0108) .and. (k .le. 0.0109)) then
    !write(*,*) "printing ISW for k:", k
    write(1,*) a, k, ISW, 2._dl*( etak - adotoa*sigma )/k
end if
!> AZ MOD END
    ...
    if (CTransScal%NumSources > 2) then
        ...
        ! EFTCAMB sources: modifications to the sources part
...
!if (.false.) then
            do w_ix = 1, num_redshiftwindows
                ...
                if (W%kind == window_lensing) then
                    ! EFTCAMB MOD START: lensing source computation
                    if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                        sources(3+w_ix) =-2*phi*W%win_lens(j)
                    else if (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive) then
                        sources(3+w_ix) = - (EFTLensing)/k*W%win_lens(j)
                    end if
                    ! EFTCAMB MOD END
                elseif (W%kind == window_counts) then
!> AZ TEST: START
!write(*,*) "computing the sources for galaxy counts. a,k:", a,k
!> AZ TEST: END
                    ...
                    ! EFTCAMB MOD START: counts source computation
                    if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
...
!AZ TEST:
!write(*,*) "counts_density: ",counts_density_source
!AZ TEST.
                    ...
                    if (counts_redshift) then
                        ! EFTCAMB MOD START: counts redshift computation
                        if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                            ...
                            counts_redshift_source = +W%ddwing2(j)/k*sigma +W%dwing2(j)/k*(2._dl*k*EFT_Psi-2._dl*adotoa*sigma)&
                                & +W%wing2(j)/k*(sigma*(adotoa**2-EV%eft_cache%Hdot)-adotoa*k*EFT_Psi +k*EFT_PsiDot)
                        end if
                        ! EFTCAMB MOD END
                        if (k>0.8e-2) then
                    ...
                    if (counts_timedelay) then
                        ! EFTCAMB MOD START: counts time delay source computation
                        if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                            !time delay; WinV is int g/chi
                            counts_timedelay_source= 2*(1-2.5*W%dlog10Ndm)*W%WinV(j)*2*phi
                        else if  (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive) then
                            counts_timedelay_source= 2*(1-2.5*W%dlog10Ndm)*W%WinV(j)*(EFT_Phi+EFT_Psi)
                        end if
                        ! EFTCAMB MOD END
                    else
                    ...
                    if (counts_ISW) then
                        ! EFTCAMB MOD START: counts ISW source computation
                        if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                            !WinF is int wingtau
                            counts_ISW_source = W%WinF(j)*2*phidot
                        else if (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive) then
                            counts_ISW_source = W%WinF(j)*(EFT_PhiDot+EFT_PsiDot)
                        end if
                        ! EFTCAMB MOD END
                    else
                        counts_ISW_source = 0
                    ...
                    if (counts_potential) then
                        ! EFTCAMB MOD START: counts potential source computation
                        if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                            !approx phi = psi
                            counts_potential_source = ( phidot/adotoa + phi +(5*W%dlog10Ndm-2)*phi ) * W%wing(j) + phi * W%wingtau(j)
                        else if (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive) then
                            counts_potential_source = ( EFT_PhiDot/adotoa + EFT_Psi +(5*W%dlog10Ndm-2)*EFT_Phi ) * W%wing(j) + EFT_Psi*W%wingtau(j)
                        end if
                        ! EFTCAMB MOD END
                    else
                    ...
                    if (counts_velocity) then
                        ! EFTCAMB MOD START: counts velocity source computation
                        if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                            ...
                            counts_velocity_source =  W%dwingtau(j)/k*sigma +W%wingtau(j)/k*(-adotoa*sigma +k*EFT_Psi) &
                                - counts_radial_source  !don't double count terms; counts_radial is part of counts_velocity with 1/H/chi
                        end if
                        ! EFTCAMB MOD END
                    else
                    ...
                    ! EFTCAMB MOD START: redshift lensing contribution
                    if (DoRedshiftLensing) then
                        if (CP%EFTCAMB%EFTflag==0 .or. .not. EV%EFTCAMBactive) then
                            sources(3+W%mag_index+num_redshiftwindows) = phi*W%win_lens(j)*(2-5*W%dlog10Ndm)
                        else if (CP%EFTCAMB%EFTflag/=0 .and. EV%EFTCAMBactive) then
                            sources(3+W%mag_index+num_redshiftwindows) = 0.5_dl*(EFT_Phi+EFT_Psi)*W%win_lens(j)*(2-5*W%dlog10Ndm)
                        end if
                    end if
                    ! EFTCAMB MOD END

!AZ TEST:
!write(*,*) "a, k, sources: ",a, k, sources(3+w_ix)
!AZ TEST.

                elseif (W%kind == window_21cm) then
!> AZ TEST: START
!write(*,*) " computing the 21cm source functions. a:",a
!> AZ TEST: END
                    if (line_basic) then
...
!> AZ TEST: START
!write(*,*) 'derivs, a, ayprime:', a, ayprime
!> AZ TEST: END
