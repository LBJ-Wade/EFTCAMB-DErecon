...
!write(*,*) "computing the designer dtauda"
            dtauda = CP%EFTCAMB%model%compute_dtauda( a, CP%eft_par_cache , eft_cache )
!write(*,*) "designer dtauda:", dtauda
        end if
    end if
    ! EFTCAMB MOD END.
...
!> tests:
!write(*,*) "calling GaugeInterface_ScalEv"
        call GaugeInterface_ScalEv(EV,y,tau,tauend,tol1,ind,c,w)
!write(*,*) "GaugeInterface_ScalEv done."
...
!> AZ MOD: START
!write(*,*) "EFTturnOnTime:",EV%EFTturnOnTime
!write(*,*) "a,Omega :",a, EV%eft_cache%EFTOmegaV
!write(*,*) "a,OmegaP:",a, EV%eft_cache%EFTOmegaP
!write(*,*) "a,OmegaPP:",a, EV%eft_cache%EFTOmegaPP
!write(*,*) "a,OmegaPPP:",a, EV%eft_cache%EFTOmegaPPP
!write(*,*) "starting the output subroutine"
!write(*,*) "a,k:",y(1),EV%k_buf
!> AZ MOD: END
...
! output of the field
!write(*,*) "output of the field"
!> test
!write(*,*) "EFTCAMB output. a,k:", a,k
!if(k .le. 7E-006 ) then
!    write(1,*) a,EV%eft_cache%pi
!    write(3,*) a,  ( EV%eft_cache%EFTpiC +k2*EV%eft_cache%EFTpiD1 +k2**2*EV%eft_cache%EFTpiD2  )/( EV%eft_cache%EFTpiA1 +k2*EV%eft_cache%EFTpiA2 )
!elseif ( k .ge. 1.02E-005 .and. k .le. 1.03E-005) then
!    write(2,*) a,EV%eft_cache%pi
!    write(4,*) a,  ( EV%eft_cache%EFTpiC +k2*EV%eft_cache%EFTpiD1 +k2**2*EV%eft_cache%EFTpiD2  )/( EV%eft_cache%EFTpiA1 +k2*EV%eft_cache%EFTpiA2 )
!end if
!write(*,*) "output of the field done"
...
!write(*,*) "RHS perurbed Einstein equations calculated:"
!write(*,*) "dgrho,dgq,dgpi:", dgrho, dgq,dgpi
        ...
        ! EFTCAMB MOD END
!write(*,*) "calculating the TT source"
...
!write(*,*) "Psi:",EV%eft_cache%Psi
            EV%eft_cache%Phi           = ( etak -EV%eft_cache%adotoa*EV%eft_cache%sigma )/k
!write(*,*) "Phi:",EV%eft_cache%Phi
            EV%eft_cache%mu            = -2._dl*k*( EV%eft_cache%sigmadot +EV%eft_cache%adotoa*EV%eft_cache%sigma)/(dgrho)
!write(*,*) "mu:",EV%eft_cache%mu
!write(*,*) "etak, adotoa, sigma, sigmadot:", etak,EV%eft_cache%adotoa,EV%eft_cache%sigma,EV%eft_cache%sigmadot
...
!write(*,*) "gamma:",EV%eft_cache%gamma            ! check the cache:
!write(*,*) "calling isNaN"
            call EV%eft_cache%is_nan( IsNaN )
!write(*,*) "isNaN called. IsNaN:", IsNaN
            if ( IsNaN ) then
...
!write(*,*) "ending output. sources:", sources
!write(*,*)
...
!write(*,*) "a:",a
!write(*,*) "computing the background functions"
                call CP%EFTCAMB%model%compute_background_EFT_functions( a, CP%eft_par_cache , EV%eft_cache )
!write(*,*) "omega:",EV%eft_cache%EFTOmegaV
!write(*,*) "omega_p:",EV%eft_cache%EFTOmegaP
!write(*,*) "omega_pp:",EV%eft_cache%EFTOmegaPP
!write(*,*)
...
!write(*,*) "    inside derivs. Printing ayprime"
!write(*,*) "    ayprime:",ayprime
...
!write(*,*) "pieces of ayprime(2):"
!write(*,*) "U,F,X:",EV%eft_cache%EFTeomU, EV%eft_cache%EFTeomF,EV%eft_cache%EFTeomX
...
!write(*,*)
!write(*,*) "pieces of the pi equation at a = ",a
!write(*,*) "A1, A2:",EV%eft_cache%EFTpiA1, EV%eft_cache%EFTpiA2
!write(*,*) "B1, B2:",EV%eft_cache%EFTpiB1, EV%eft_cache%EFTpiB2
!write(*,*) "C:",EV%eft_cache%EFTpiC
!write(*,*) "D1, D2:",EV%eft_cache%EFTpiD1, EV%eft_cache%EFTpiD2
!write(*,*) "H0 E:",EV%eft_cache%EFTpiE*CP%eft_par_cache%h0_Mpc
!write(*,*) "Z:", EV%eft_cache%z
!write(*,*)
!if (EV%eft_cache%z > 1.d0 .or. EV%eft_cache%z< -1.d0) then
!    pause
!end if
