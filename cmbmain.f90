...
!AZ TEST:
!write(*,*) "Initializing powers. Error:",global_error_flag
!AZ TEST.
...
!AZ TEST:
!write(*,*) "Initializing Transfers. Error:",global_error_flag
!AZ TEST.
...
!AZ TEST:
!write(*,*) "Computing Sources. Error:",global_error_flag
!AZ TEST.
...
!AZ TEST:
!write(*,*) "Computing transfer functions. Error:",global_error_flag
!AZ TEST.
...
!AZ TEST:
!write(*,*) "exiting cmbmain. Error:", global_error_flag
!AZ TEST.
                ...
                do j= i, num_redshiftwindows
                    ...
                    if (CTrans%limber_l_min(s_ix2) /=0) then
                        ...
                        do ell =  max(CTrans%limber_l_min(s_ix), CTrans%limber_l_min(s_ix2)), Ctrans%ls%l0
                            ...
                            if (i>=1) then
                                if (Redshift_w(i)%kind == window_lensing) then
                                    fac=fac/2*reall*(reall+1)
                                end if
                            end if
    ...
    do i =0, num_redshiftwindows
        ...
        if (i==0) then
            if (CP%Want_CMB_lensing) max_bessels_etak = max(max_bessels_etak, min(CP%Max_eta_k,ell_needed*25._dl*AccuracyBoost))
        else
    ...
    call GetNumEqns(EV)
!AZ TEST:
!write(*,*) "starting CalcScalarSources, k, error:", EV%q,global_error_flag
!write(*,*) "k step: ", q_ix,"/", size(Evolve_q%points)
!AZ TEST.
...
!AZ TEST:
!write(*,*) "ending CalcScalarSources, k, error:", EV%q,global_error_flag
!AZ TEST.
...
!> AZ TEST: START
!write(*,*) "calling GaugeInterface_EvolveScal. a:",y(1)
!> AZ TEST: END
            call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
!> AZ TEST: START
!write(*,*) "calling output. a:",y(1)
!> AZ TEST: END
...
101         if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
    ...
    if (.not. Do21cm .and. Evolve_q%points(ik)/(CP%H0/100) >  Min_kh_nonlinear) then
    ...
    do j=1,max_bessels_l_index
        ...
        tmin=max(TimeSteps%points(2),tmin)
        if (.not. CP%Want_CMB .and. .not. CP%Want_CMB_lensing) tmin = max(tmin,tau_start_redshiftwindows)
